...
class AIResponseThread(QThread):
    ...
    def run(self):
        try:
            response = ollama.chat(model="gemma:2b", messages=[{"role": "user", "content": self.query}])
...
class ChatApp(QWidget):
    ...
    def check_predefined_responses(self, user_message):
        responses = {
            ...
            # Date and Time
            ("what time is it", "what's the time", "current time"): "The current time is: [Provide dynamic time here]",
            ("what's the date", "what is the date", "current date"): "Today is: [Provide dynamic date here]",
